...
import com.gistone.bjhky.SawggerModel.Destination;
import com.gistone.bjhky.annotation.PassToken;
import com.gistone.bjhky.entity.*;
import com.gistone.bjhky.pkname.Swagger;
import com.gistone.bjhky.service.IDestinationsManagerService;
import com.gistone.bjhky.service.ISt4ScsChService;
import com.gistone.bjhky.service.ISt4ScsCpService;
...
public class DestinationsManagerController {
    ...
    @Autowired
    private ISt4ScsCpService cpService;//获取角色与航点数据关系字段
    ...
    public Result insertDestinations(HttpServletRequest request,
        ...
        if (data == null) {
            return Result.build(1001, "添加信息不能为空");
        }
        ...
        //下面这个是从集合里面取到每个航点的核查的进展及图片信息
        /*if(des.getDestination()!=null){
            if(des.getDestination().size()>0){
                List<Destination> desList = des.getDestination();
                for (Destination de:desList) {
                    if(ObjectUtils.isNotNullAndEmpty(de.getSt4ScsCk())){

                    }
                    scscnList.add(de.getSt4ScsCn());
                }
            }
        }*/
        ...
        for (int i = 0; i < destinationlist.size(); i++) {
            ...
            if (scscc.getCc002() == null) {
                return Result.build(1001, "添加航点信息错误");
            }
            if (scscc.getCc003() == null) {
                return Result.build(1001, "航点类型不能为空");
            }
            if (scscc.getCc012() == null || "".equals(scscc.getCc012())) {
                return Result.build(1001, "航点名称不能为空");
            }
            ...
            if (scscc.getCc003() == 0) {//巡护表
                ...
                if (cf == null) {
                    return Result.build(1001, "添加巡护信息不能为空");
                }
...
//                    new RuntimeException("插入航点信息错误");
                ...
                if(fields != null){
                    ...
                    if (!"".equals(ret.get())) {
                        return Result.build(1001, ret.get());
                    }
                    //设置字符串为空
                ...
                if (ck == null) {
                    return Result.build(1001, "添加核查信息不能为空");
                }
                if (ck.getCd004() == null || "".equals(ck.getCd004())) {
//                    new RuntimeException("插入航点信息错误");
                    return Result.build(1001, "问题点编号不能为空，添加航点信息错误");
                }
...
//                new RuntimeException("插入航点信息错误");
    ...
    public Result insertDestinationsRecord(HttpServletRequest request, @RequestBody @ApiParam(name = "巡护信息添加", value = "json格式", required = true) Swagger<Destinations> data) {
        Destinations des  = data.getData();
        //获取巡护任务的唯一标识
        //Object taskId = des.getDestination().get(0).getTaskId();
        //获取用户id
        ...
        if (scsCy.getCy002() == null || "".equals(scsCy.getCy002())) {
            return Result.build(1001, "添加巡护记录名称不能为空");
        }
        if (scsCy.getCy003() == null || "".equals(scsCy.getCy003())) {
            return Result.build(1001, "添加巡护记录开始时间不能为空");
        }
        if (scsCy.getCy004() == null || "".equals(scsCy.getCy004())) {
            return Result.build(1001, "添加巡护记录结束时间不能为空");
        }
        if (scsCy.getCy005() == null || "".equals(scsCy.getCy005())) {
            return Result.build(1001, "添加巡护记录类型不能为空");
        }
        if (scsCy.getSa001() ==null) {
            return Result.build(1001, "添加巡护记录巡护人员信息不能为空");
        }
        if (scsCy.getCy009() == null || "".equals(scsCy.getCy009())) {
            return Result.build(1001, "添加巡护记录数据提交类型不能为空");
        }
        if (scsCy.getCy011() == null || "".equals(scsCy.getCy011())) {
            return Result.build(1001, "添加巡护记录巡护任务类型不能为空");
        }
        LocalDateTime date = LocalDateTime.now();//获取当前时间
...
//                    new RuntimeException("插入航点信息错误");
                    ...
                    List<Map<String, Object>> fields = cpService.getSt4ScsCpInfo(ck.getCk091().toString(), 0);//1代表巡护0代表核查
                    //判断必填字段是否填写
                    AtomicReference<String> ret = new AtomicReference<>("");
                    fields.stream().forEach(objectMap -> ret.set(fieldJudge(null, ck, objectMap)));
                    if (!"".equals(ret.get())) {
...
//                new RuntimeException("插入航点信息错误");
    ...
    public Result sendRealPositon( @RequestBody @ApiParam(name = "", value = "json格式", required = true)  Swagger<St4ScsCh> data
    ) {

        St4ScsCh param = data.getData();

        if(!ObjectUtils.isNotNullAndEmpty(param.getCh002())){
            return Result.build(1001,"巡查人员ch002"+ResultMsg.MSG_1001);
        }
        if(!ObjectUtils.isNotNullAndEmpty(param.getCh003())){
            return Result.build(1001,"经度CH003"+ResultMsg.MSG_1001);
        }
        if(!ObjectUtils.isNotNullAndEmpty(param.getCh004())){
            return Result.build(1001,"纬度CH004"+ResultMsg.MSG_1001);
        }
        if(!ObjectUtils.isNotNullAndEmpty(param.getCh005())){
            return Result.build(1001,"人员状态ch005"+ResultMsg.MSG_1001);
        }
