...
//@Value("${swaggerState}")
private boolean swaggerState=true;
    ...
    public Docket createRestApi() {
        return new Docket(DocumentationType.SWAGGER_2).select().apis(RequestHandlerSelectors.any())
                .paths(PathSelectors.ant("//**")).build().groupName("安徽红线接口文档V1.1").pathMapping("/")
                .apiInfo(apiInfo("安徽红线接口文档", "文档中可以为整个系统所有接口", "1.1"))
                //.globalResponseMessage(RequestMethod.GET, responseMessageList)
                .globalResponseMessage(RequestMethod.POST, responseMessageList()).enable(swaggerState);
